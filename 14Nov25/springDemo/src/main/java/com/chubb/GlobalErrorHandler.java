...
@RestControllerAdvice  
                         //tells spring if something goes wrong come here and print msg in human readabel format
...

{
	...
	public Map<String,String> handlerException(MethodArgumentNotValidException exception) //to store fieldName->errorMessage
	{
		...
		errorsList.forEach((error)->
		{
			...
			String message = error.getDefaultMessage(); //the message defined in @Valid annotation in corresponding class
			errorMap.put(fieldName, message);  //Put in map --> { "price": "must be >= 1" }
